...
add_action( 'widgets_init', 'agpta_homepage_sidebar' );
...
function send_ticket_email( $email, $event_name, $amount ) {
	...
	if ( $attachment_path ) {
		wp_mail( $email, $subject, $body, $headers, array( $attachment_path ) );
...
function generate_pdf_ticket( $event_name, $amount ): string {
	...
	$path = trailingslashit( $upload_dir['basedir'] . '/ticket-' . uniqid( '', true ) ) . '.pdf';
	file_put_contents( $path, $dompdf->output() );

	if ( ! $wp_filesystem->put_contents( $path, $dompdf->output(), FS_CHMOD_FILE ) ) {
...
function agpta_homepage_sidebar() {
	register_sidebar(
		array(
			'name'          => __( 'Homepage Sidebar', 'agpta' ),
			'id'            => 'homepage-sidebar',
			'description'   => __( 'Widgets in this area will appear on the homepage.', 'text_domain' ),
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h2 class="agpta-homepage-sidebar-widget-header">',
			'after_title'   => '</h2>',
		)
	);
}
...
function get_board_members() {
	...
	if ( ! empty( $posts ) ) {
		foreach ( $posts as $index => $post ) {
			...
			$members[] = array(
				'id'     => $post->ID,
				'name'   => $meta['_member_name'][0],
				'role'   => $meta['_member_role'][0],
				'status' => $meta['_member_status'][0],
