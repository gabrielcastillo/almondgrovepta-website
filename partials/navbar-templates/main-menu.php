...
function render_menu_item( $menu_item, $current_slug ) {
    ...
    if ( isset( $menu_item['sub_menu'] ) && !empty( $menu_item['sub_menu'] ) ) {
        ...
        echo '<a href="' . esc_url( $menu_item['url'] ) . '" class="' . esc_attr( $active_class ) . ' text-gray-800 hover:bg-red-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium" role="menuitem">' . esc_html($menu_item['title']) . '</a>';
    }
}
...
function render_dropdown_menu( $menu_item, $active_class ) {
    global $current_slug;
    $slug = $menu_item['slug'] ?? '';
    $menu_id = strtolower(preg_replace( '/[^a-z0-9_\-]/', '-', $menu_item['title'] ));
    echo '<div class="relative ml-3">';
    echo '<div class="relative group">';
    echo '<button id="' . esc_attr($menu_id) . '-trigger" data-menu-id="' . esc_attr($menu_id) . '" aria-controls="'. esc_attr($menu_id) .'" type="button" class="has-dropdown inline-flex items-center text-gray-800 hover:bg-red-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium" aria-expanded="false" aria-haspopup="true">';
    ...
    echo '</button>';
    echo '</div>';
    echo '<div style="display:none;" data-menu="' . esc_attr($menu_id) . '" class="' . esc_attr($active_class) . ' dropdown-menu absolute z-10 mt-2 w-44 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="' . esc_attr($menu_id) . '-trigger" aria-hidden="true" tabindex="-1">';
